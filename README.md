...
Present any business issue and receive structured, multi-perspective analysis with engineered dissent -- not consensus from a single voice, but a decision that shows where expert perspectives collide and why. CDP emulates an SMB executive committee: a CEO frames and routes, C-suite executives analyze through domain lenses, specialist team leads produce findings, and the CEO synthesizes a decision that addresses the strongest objections.

Operates at three engagement tiers (hallway question, working session, board meeting) and five synthesis modes (Guardian, Pioneer, Architect, Analyst, Sentinel). Runs as a [Claude Code](https://docs.anthropic.com/en/docs/claude-code) agent skill.
...
**Prerequisite:** [Claude Code CLI](https://docs.anthropic.com/en/docs/claude-code) installed and configured.
...
Not sure which tier to use? Let the CEO assess:
```
...
| **Production artifacts** | Never | Optional (`--produce`) | Always |
...
See [`config/company-profile.md`](.claude/skills/corporate-decision-panel/config/company-profile.md) for full specification.
...
See [`config/routing-table.md`](.claude/skills/corporate-decision-panel/config/routing-table.md) for full specification.
...
|------|--------------|--------|------------|
| Tier 1 | Advisory Note | 3-5 sentences | Never |
...
See the [`templates/`](.claude/skills/corporate-decision-panel/templates/) directory for full output format specifications.
...
├── .claude/
│   ├── agents/
│   │   ├── ceo.md                              # CEO: 5-phase cascade protocol (Opus)
│   │   ├── c-suite/                            # 8 C-suite agent definitions (Sonnet)
│   │   │   ├── cao.md
│   │   │   ├── cfo.md
│   │   │   ├── ciso.md
│   │   │   ├── coo.md
...
│   │   │   ├── vp-delivery.md
│   │   │   └── vp-sales.md
│   │   └── team-leads/                         # 34 team lead subagent definitions (Haiku)
│   │       ├── cao/                            # 4 leads: HR, Legal, Admin, Comms
│   │       ├── cfo/                            # 5 leads: Controller, FP&A, Treasury, AP/AR, Tax
│   │       ├── ciso/                           # 4 leads: SecOps, GRC, IAM, SecArch
│   │       ├── coo/                            # 4 leads: Ops, Process, Vendor, Facilities
│   │       ├── cso/                            # 5 leads: Market, Competitive, Tech, Regulatory, Precedent
│   │       ├── cto/                            # 4 leads: Engineering, DevOps, Data, Product
│   │       ├── vp-delivery/                    # 4 leads: PM, Resource, Client, QA
│   │       └── vp-sales/                       # 4 leads: Sales Ops, Accounts, BizDev, Enablement
│   ├── commands/
│   │   └── cdp/                                # Slash command definitions
...
│           ├── config/
│           │   ├── company-profile.md          # Archetype presets and overrides
│           │   ├── decision-modes.md           # Five mode definitions with CEO prompt modifiers
│           │   └── routing-table.md            # Decision-type routing defaults and thresholds
│           └── templates/
│               ├── advisory-note.md            # Tier 1 output format
│               ├── panel-assessment.md         # Tier 2 output format
│               ├── decision-record.md          # Tier 3 output format (9 sections)
│               ├── comparative-decision-record.md  # Multi-mode variant
│               ├── company-context.md          # Company brief template (user fills in)
│               └── production/                 # Production artifact specifications
│                   ├── board-document.md        # DOCX structure spec
│                   ├── board-presentation.md    # PPTX structure spec
│                   ├── capsule-structure.md     # PDF layered archive spec
│                   └── decision-briefing-page.md  # HTML page spec
├── ref/
│   ├── design/                                 # Design documentation
│   └── ideation/                               # Research archive and session records
├── .cdp-context/                               # User company data (gitignored)
│   └── company.md
├── .cdp-output/                                # Production output (gitignored)
│   └── YYYY-MM-DD_<issue-slug>/
└── .gitignore
```
...
The `ref/ideation/` directory contains the research archive from the design process, including session records, idea reports, and source materials. These are historical records, not operational specifications.

For detailed specifications, see the config and template files:
- [SKILL.md](.claude/skills/corporate-decision-panel/SKILL.md) -- Complete skill specification
- [config/decision-modes.md](.claude/skills/corporate-decision-panel/config/decision-modes.md) -- Five mode definitions with full CEO prompt modifiers
- [config/routing-table.md](.claude/skills/corporate-decision-panel/config/routing-table.md) -- Routing defaults and threshold conditions
- [config/company-profile.md](.claude/skills/corporate-decision-panel/config/company-profile.md) -- Archetype presets and override mechanism
- [templates/](.claude/skills/corporate-decision-panel/templates/) -- All output format and production artifact specifications
